 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const jT="firebase-heartbeat-database",HT=1,Ri="firebase-heartbeat-store";let Vl=null;function Fm(){return Vl||(Vl=aT(jT,HT,{upgrade:(t,e)=>{switch(e){case 0:try{t.createObjectStore(Ri)}catch(n){console.warn(n)}}}}).catch(t=>{throw cr.create("idb-open",{originalErrorMessage:t.message})})),Vl}async function zT(t){try{const n=(await Fm()).transaction(Ri),r=await n.objectStore(Ri).get(Um(t));return await n.done,r}catch(e){if(e instanceof jn)Mn.warn(e.message);else{const n=cr.create("idb-get",{originalErrorMessage:e==null?void 0:e.message});Mn.warn(n.message)}}}async function qd(t,e){try{const r=(await Fm()).transaction(Ri,"readwrite");await r.objectStore(Ri).put(e,Um(t)),await r.done}catch(n){if(n instanceof jn)Mn.warn(n.message);else{const r=cr.create("idb-set",{originalErrorMessage:n==null?void 0:n.message});Mn.warn(r.message)}}}function Um(t){return`${t.name}!${t.options.appId}`}/**
 * @license
 * Copyright 2021 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
